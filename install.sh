...
download_oe_code() {
  ...
  if [ ! -d $oe_download ]
  then
    git clone $OE_REPO $oe_download
    report_success $? "git clone of OE"
    cd $oe_download
    do_git_checkout $GIT_BRANCH $GIT_TRACK
    cd -
  else
    log "Download already exists; remove directory $oe_download before continuing."
    exit 1
  fi
}
...
print_help() {
  ...
  # print options common to dev and install scripts:
  print_base_help
  echo ""
  ...
  echo "    Depends: -i"
  echo "  -l: Create and link OE image galleries. Image galleries are created"
  echo "    in $OE_BASE_GALLERIES_DIR and (soft) linked to the"
  echo "    $SITE_DIR/$OE_DIR/images/ directory. Although this is used by the"
  echo "    Mirth ESB, it is also used by vendors wishing to display stereo"
  echo "    and/or VFA images via the OpenEyes UI. Depends: -c. Uses: -O"
...
# 
while getopts ":guitdayeocslmhRQL:W:P:Y:G:S:O:D:T:Z:U:C:N:" opt; do
  case $opt in
    W)
    ...
    N)
      ...
      ;;
    B)
      #echo "Changing GIT branch from $GIT_BRANCH to: $OPTARG" >&2
      #echo "Only used with -p."
      #GIT_BRANCH="$OPTARG"
      ;;
    C)
